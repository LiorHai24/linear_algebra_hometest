...
def histogram(page_rank_dict, trails):
    ...
    for key, value in page_rank_dict.items():
        probability_vec[key -1] = value / (trails)
    return probability_vec


def rand_walk_loop(G, n, stat_vec):
    ...
    for steps in range(2, 7): #the number of steps is
        N = pow(2, steps)
        print("the current steps number is ", N)
        for d in range(2, 7):#the wanted delta
            wanted_delta = pow(2, -d)
            cover_time = 0
